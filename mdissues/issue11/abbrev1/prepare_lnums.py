...
def init_lnums(filein):
 ...
 print(len(recs),"lnums read from",filein)
 d = {}
 for rec in recs:
  lnum = rec.lnum
  if lnum in d:
   print('init_lnum: unexpected duplicate',lnum)
  d[lnum] = rec
 return recs,d
...
class Link(object): # unused
 def __init__(self,x):
  pass
 
def prepare(lines,dlnum):
 newlines = []
 links = []
 inentry = False
 for iline,line in enumerate(lines):
  if line.startswith('<L>'):
   ...
   newline = line
   if linklines != []:
    links.append((metaline,linklines))
   inentry = False
  elif not inentry:
   newline = line
  elif line.strip() == '':
   ...
   testline = re.sub(r'<ab>N\.</ab>','<ab>N_.</ab>',line)
   newline = line  
   if testline != line:
    lnum = iline + 1
    if lnum in dlnum:
     newline = testline
     linklines.append(newline)
  newlines.append(newline)
     
 return newlines,links

def write_newlines(fileout,newlines):
 outarr = newlines
 with codecs.open(fileout,"w","utf-8") as f:
   for out in outarr:
    f.write(out+'\n')  
...
def get_href(metaline):
 m = re.search(r'<pc>(.*?)<',metaline)
 pc = m.group(1)
 href = 'https://www.sanskrit-lexicon.uni-koeln.de/scans/csl-apidev/servepdf.php?dict=MD&page=%s' % pc
 return href,pc

def write_links(fileout,links):
 outrecs = []
 for link in links:
  outarr = []
  metaline,linklines = link
  href,pc = get_href(metaline)
  linkref = '[[%s][%s]]' %(href,pc)
  outarr.append('* %s' % linkref)
  outarr.append(metaline)
  for linkline in linklines:
   outarr.append(linkline)
...
if __name__=="__main__":
 ...
 filein1 = sys.argv[2] # File with line numbers
 fileout = sys.argv[3] # temp version of xxx.txt
 fileout1 = sys.argv[4] # org file with links
 lines = read_lines(filein)
 print(len(lines),"lines from",filein)
 recs,d = init_lnums(filein1)
 newlines,links = prepare(lines,d)
 print(len(links),"links found")
 
 write_newlines(fileout,newlines)
 write_links(fileout1,links)
