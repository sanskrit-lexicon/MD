"""
Markup fixer + audit for md.txt (MD).
Counterpart of PWG#175, PWK#113, and siblings.

MD-specific notes: 0 whitespace hits. 16 non-standard <ab n> (compass dirs, Latin).

Inputs: ../../../csl-orig/v02/md/md.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "md" / "md.txt", HERE / "md.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "md_fixed.txt"
...
TRIM_TAGS = ['lex', 'ab', 'hom', 'cl', 'per', 'bot', 'lang', 'ls', 'zoo', 'gk', 'lat', 'fr']
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(lex|ab|hom|cl|per|bot|lang|ls|zoo|gk|lat|fr)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for md.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"MD markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against md.txt.\n")
        f.write(f"Paired tags: <lex>, <ab>, <hom>, <cl>, <per>, <bot>, <lang>, <ls>, <zoo>, <gk>, <lat>, <fr>\n")
        f.write(f"Notes: 0 whitespace hits. 16 non-standard <ab n> (compass dirs, Latin).\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <lex>, <ab>, <hom>, <cl>, <per>, <bot>, <lang>, <ls>, <zoo>, <gk>, <lat>, <fr>\n\n")
